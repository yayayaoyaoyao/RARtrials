...
In this formula, $\gamma$ is a tuning parameter with commonly used values $1, 0.5, \mbox{and } n/2N$. The posterior probabilities $p_1,...,p_K$ are often restricted to $[e,1-e]$ for $0<e<1$. The decision rules of this method include three parts [@Wathen2017]:  
...
sim4<-lapply(1:1000,function(x) {
    sim_brar_known_var(Pats=10,nMax=50000,TimeToOutcome=expression(rnorm(
      length(vStartTime ),30, 3)),enrollrate=0.1,N1=192,armn=3,
            a_U=c(0.98414,0.98414),N2=1920,tp=1,armlabel=c(1,2,3),blocksize=6,
...
decision<-t(sapply(sim5, "[[", 1))
sum(decision[,2]=='Superiorityfinal')/1000 #0.882
#The simulated power from 1000 simulations is 0.882.
```


It can be observed from the above examples that continuous outcomes with both known and unknown variances produce the similar results of power.
...
In this formula, $P_m$ is the posterior mean, $$\Pi=\{ a_{k,t-1}| 0\leq k\leq K , \sum_{k=0}^K a_{k,t}=1\}$$ is a family of allocation rules, $$\tilde{x}_0=(x_{k,0})^K_{k=0}$$ is the initial state considering the prior information, $d$ is a discount factor and $E^\pi[.]$ is the average responses of participants
attained with prior information, and our objective is to maximize this value.

The optimal solution using Gittin index is given by 
...
set.seed(123452)
stopbound7<-lapply(1:1000,function(x){flgi_cut_off_known_var(
...
stopbound7a<-do.call(rbind,stopbound7)
sum(stopbound7a[,1]>2.074 | stopbound7a[,2]> 2.074)/1000 #0.05
...
set.seed(123452)
sim7<-lapply(1:1000,function(x){sim_flgi_known_var(Gittinstype='KV',
